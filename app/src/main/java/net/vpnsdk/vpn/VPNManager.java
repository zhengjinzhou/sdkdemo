...
import net.vpnsdk.vpn.AAAMethodItem;
import net.vpnsdk.vpn.NativeCallback;
import net.vpnsdk.vpn.NativeLib;
...
//				Integer.MAX_VALUE, VPNAccount.getDefaultFlag(), this);
	...
	public int startVPN(VPNAccount account) {
		if (account.getCanceled()) {
			return Common.VpnError.ERR_INTERRUPTED;
		}
		mAuthInput = account;
...
//				Integer.MAX_VALUE, Integer.MAX_VALUE, auth.getFlag(), this);
	...
	public class AAAMethod {
		...
		private ArrayList<InputField> mList = new ArrayList<InputField>();
	...
	public void startVpnActivity() {
		...
		if (Common.VpnStatus.IDLE == getStatus() && (!mStartingL3Vpn)) {
